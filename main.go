...
import (
	...
	"chatgpt-challenge/internal/repository/laptop"
	"chatgpt-challenge/internal/repository/prompt"
...
const laptopSchemaSystemMessage = "You are an expert in structured data extraction. You will be provided with unstructured text describing a laptop and you should convert it into the specified structure. Fill in any missing information that is required and disregard any extra details."
...
func startLaptopWorkerQueue(repo laptopworkerqueue.Repository, promptRepo laptopworkerqueue.PromptRepository, laptopStructifySvc laptopworkerqueue.LaptopStructifyService) {
	prompts := []string{
	...
	for _, prompt := range prompts {
		laptopWorkerQueue.Enqueue(laptopworkerqueue.Task{
			PromptID:      sha256.Sum256([]byte(prompt)),
